...
# Use ld.lld when building with LTO, or for debug builds, if available. FIXME:
# With CMake 3.22+ full conditional syntax can be used in
...
separate_arguments(
  LD_VERSION_COMMAND UNIX_COMMAND
...
message(
  STATUS "  Linker supports split debug info - ${LD_SUPPORTS_SPLIT_DEBUG}")
message(STATUS "  Linker supports --gdb-index - ${LD_SUPPORTS_GDB_INDEX}")
message(
  STATUS
    "  Linker supports --disable-new-dtags - ${LD_SUPPORTS_DISABLE_NEW_DTAGS}")
...
message(
  STATUS "  Archiver supports thin archives - ${AR_SUPPORTS_THIN_ARCHIVES}")
...
# passing the option DT_RUNPATH is used, which can be overriden by the value of
# LD_LIBRARY_PATH set in the environment, resulting in unexpected behaviour for
# developers.
...
# Prefer thin archives by default if they can be both created by the archiver
# and read back by the linker.
...
option(USE_THIN_ARCHIVES "Produce all static libraries as thin archives"
       ${USE_THIN_ARCHIVES_DEFAULT})
...
if(ENABLE_DEVELOPER_MODE
  ...
  webkit_check_compiler_flags(CXX CXX_COMPILER_SUPPORTS_GSPLIT_DWARF
                              -gsplit-dwarf)
...
# Enable the usage of OpenMP. - At this moment, OpenMP is only used as an
# alternative implementation to native threads for the parallelization of the
# SVG filters.
if(USE_OPENMP)
...
set(ENABLE_ASSERTS
    "AUTO"
    CACHE STRING "Enable or disable assertions regardless of build type")
set_property(CACHE ENABLE_ASSERTS PROPERTY STRINGS "AUTO" "ON" "OFF")

if(ENABLE_ASSERTS STREQUAL "AUTO")
  # The default value is handled by the NDEBUG define which is generally set by
  # the toolchain module used by CMake.
...
# Check whether features.h header exists. Including glibc's one defines
# __GLIBC__, that is used in Platform.h
...
# Check for functions _GNU_SOURCE=1 is required to expose statx
...
webkit_check_have_struct(HAVE_STAT_BIRTHTIME "struct stat" st_birthtime
                         sys/stat.h)
