...
# CPACK_PACKAGE_NAME to WebKit-${PORT} - CPACK_SOURCE_IGNORE_FILES to a known
# pattern of good files
#
# The following variables affect the behavior of packaging: -
# WEBKIT_CPACK_ALL_PORTS if defined and true, will not limit packaging to just
# include files of the port (affects CPACK_SOURCE_IGNORE_FILES, just if this
# variable was not defined before). - WEBKIT_CPACK_ADD_TESTS if defined and
# true, will also add tests (affects CPACK_SOURCE_IGNORE_FILES, just if this
# variable was not defined before) - WEBKIT_CPACK_ADD_TOOLS if defined and true,
# will also add tools (affects CPACK_SOURCE_IGNORE_FILES, just if this variable
...
if(NOT DEFINED CPACK_SOURCE_IGNORE_FILES)
  set(CPACK_SOURCE_IGNORE_FILES
      ...
      "/Examples/"
      "/Websites/"
      # Other build systems: - Makefiles (.mk/Makefile)
      "\\\\.mk$"
      "\\\\.make$"
      "Makefile"
      # * XCode (Mac)
  ...
  if(NOT WEBKIT_CPACK_ADD_TESTS)
    list(APPEND CPACK_SOURCE_IGNORE_FILES "/LayoutTests/" "/ManualTests/"
         "/tests/")
  endif(NOT WEBKIT_CPACK_ADD_TESTS)

  if(NOT WEBKIT_CPACK_ADD_TOOLS)
    list(APPEND CPACK_SOURCE_IGNORE_FILES "/Tools/" "/manual-tools/" "/tools/"
         "/PageLoadTools/")
  ...
  if(NOT WEBKIT_CPACK_ALL_PORTS)
    ...
    foreach(_pattern ${FILE_PATTERNS_UNKNOWN_PORTS})
      ...
      if(_pattern_index GREATER -1)
        message(
          "pattern ${_pattern} declared of 'no-port' is actually used by ${PORT}"
        )
      else()
    ...
    foreach(_port ${ALL_PORTS})
      if(NOT ${_port} STREQUAL ${PORT})
        foreach(_pattern ${FILE_PATTERNS_${_port}})
          ...
          if(_pattern_index GREATER -1)
            message(
              "pattern ${_pattern} of port ${_port} is also used by ${PORT}")
          else()
            list(APPEND CPACK_SOURCE_IGNORE_FILES ${_pattern})
          endif()
        endforeach()
