...
#
# CMake provides Find modules that are used within WebKit. However there are
# ports where the default behavior does not work and they need to explicitly set
# their values. There are also targets available in later versions of CMake
...
# CMake provided targets. Remove wrappers whenever the minimum version is
# bumped.
#
# ICU::<C> : need to be kept for Apple ICU LibXslt::LibXslt: since 3.18
...
macro(find_package package)
  ...
  # Create targets that are present in later versions of CMake or are referenced
  # above
  if("${package}" STREQUAL "ICU")
    if(ICU_FOUND AND NOT TARGET ICU::data)
      ...
      set_target_properties(
        ICU::data
        PROPERTIES IMPORTED_LOCATION "${ICU_DATA_LIBRARY}"
      ...
      set_target_properties(
        ICU::i18n
        PROPERTIES IMPORTED_LOCATION "${ICU_I18N_LIBRARY}"
                   INTERFACE_INCLUDE_DIRECTORIES "${ICU_INCLUDE_DIRS}"
                   IMPORTED_LINK_INTERFACE_LANGUAGES "CXX")
      ...
      set_target_properties(
        ICU::uc
        PROPERTIES IMPORTED_LOCATION "${ICU_UC_LIBRARY}"
                   INTERFACE_INCLUDE_DIRECTORIES "${ICU_INCLUDE_DIRS}"
                   IMPORTED_LINK_INTERFACE_LANGUAGES "CXX")
    endif()
  elseif("${package}" STREQUAL "LibXslt")
    if(LIBXSLT_FOUND AND NOT TARGET LibXslt::LibXslt)
      ...
      set_target_properties(
        LibXslt::LibXslt
        PROPERTIES IMPORTED_LOCATION "${LIBXSLT_LIBRARIES}"
    ...
    if(LIBXSLT_EXSLT_LIBRARY AND NOT TARGET LibXslt::LibExslt)
      ...
      set_target_properties(
        LibXslt::LibExslt
        PROPERTIES IMPORTED_LOCATION "${LIBXSLT_EXSLT_LIBRARY}"
                   INTERFACE_INCLUDE_DIRECTORIES "${LIBXSLT_INCLUDE_DIR}")
    endif()
  endif()
endmacro()
