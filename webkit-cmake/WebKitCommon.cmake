...
# WebCore, WebKit) in order to allow scripts to build only part of WebKit. We
# want to run this file only once.
# -----------------------------------------------------------------------------
if(NOT HAS_RUN_WEBKIT_COMMON)
  ...
  if(DEVELOPER_MODE AND (CMAKE_GENERATOR MATCHES "Makefile" OR CMAKE_GENERATOR
                                                               MATCHES "Ninja"))
  ...
  # -----------------------------------------------------------------------------
  set(ALL_PORTS GTK JSCOnly Mac PlayStation WPE Win)
  set(PORT
      "NOPORT"
      CACHE STRING "choose which WebKit port to build (one of ${ALL_PORTS})")
  ...
  if(${RET} EQUAL -1)
    if(APPLE)
      ...
      message(
        FATAL_ERROR
          "Please choose which WebKit port to build (one of ${ALL_PORTS})")
    endif()
  endif()
  ...
  if(${CMAKE_CXX_COMPILER_ID} STREQUAL "GNU")
    if(${CMAKE_CXX_COMPILER_VERSION} VERSION_LESS "11.2.0")
      message(
        FATAL_ERROR
          "GCC 11.2 or newer is required to build WebKit. Use a newer GCC version or Clang."
      )
    endif()
  endif()
  ...
  if(LOWERCASE_CMAKE_SYSTEM_PROCESSOR MATCHES
     "(^aarch64|^arm64|^cortex-?[am][2-7][2-8])")
    set(WTF_CPU_ARM64 1)
  ...
  elseif(LOWERCASE_CMAKE_SYSTEM_PROCESSOR MATCHES "(x64|x86_64|amd64)")
    # FORCE_32BIT is set in the build script when --32-bit is passed on a
    # Linux/intel 64bit host. This allows us to produce 32bit binaries without
    # setting the build up as a crosscompilation, which is the only way to
    # modify CMAKE_SYSTEM_PROCESSOR.
    if(FORCE_32BIT)
      set(WTF_CPU_X86 1)
    else()
  ...
  # -----------------------------------------------------------------------------
  option(SHOULD_INSTALL_JS_SHELL
         "generate an installation rule to install the built JavaScript shell")
  ...
  find_package(
    PerlModules
    COMPONENTS English FindBin JSON::PP
    REQUIRED)
  ...
  find_package(
    ...
    REQUIRED)
  # Set the variable with uppercase name to keep compatibility with code and
  # users expecting it.
  set(PYTHON_EXECUTABLE
      ${Python_EXECUTABLE}
      CACHE FILEPATH "Path to the Python interpreter")

  # We cannot check for RUBY_FOUND because it is set only when the full package
  # is installed and the only thing we need is the interpreter. Unlike Python,
  # cmake does not provide a macro for finding only the Ruby interpreter.
  ...
  # Check gperf after including OptionsXXX.cmake since gperf is required only
  # when ENABLE_WEBCORE is true, and ENABLE_WEBCORE is configured in
  # OptionsXXX.cmake.
  if(ENABLE_WEBCORE)
