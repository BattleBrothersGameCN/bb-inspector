# Checks whether all the given compiler flags are supported by the compiler. The
# _compiler may be either "C" or "CXX", and the result from the check will be
# stored in the variable named by _result.
function(WEBKIT_CHECK_COMPILER_FLAGS _compiler _result)
  ...
  foreach(_flag IN LISTS ARGN)
    # If an equals (=) character is present in a variable name, it will not be
    # cached correctly, and the check will be retried ad nauseam.
    ...
    elseif(${_compiler} STREQUAL C)
      ...
      message(
        WARNING "WEBKIT_CHECK_COMPILER_FLAGS: unknown compiler '${_compiler}'")
      return()
    endif()
    if(NOT ${_cachevar})
      return()
...
# Appends, prepends, or sets (_op = APPEND, PREPEND, SET) flags supported by the
# C or CXX _compiler to a string _variable.
function(WEBKIT_VAR_ADD_COMPILER_FLAGS _compiler _op _variable)
  if(NOT
     (_op STREQUAL APPEND
      OR _op STREQUAL PREPEND
      OR _op STREQUAL SET))
...
# Prepends flags to CMAKE_C_FLAGS and CMAKE_CXX_FLAGS if supported by the C or
# C++ compiler, respectively. Almost all flags should be prepended to allow the
...
# Appends flags to COMPILE_OPTIONS of _subject if supported by the C or CXX
# _compiler. The _subject argument depends on its _kind, it may be a target name
# (with TARGET as _kind), or a path (with SOURCE or DIRECTORY as _kind).
function(WEBKIT_ADD_COMPILER_FLAGS _compiler _kind _subject)
  foreach(_flag IN LISTS ARGN)
    webkit_check_compiler_flags(${_compiler} flag_supported "${_flag}")
    if(flag_supported)
      set_property(${_kind} ${_subject} APPEND PROPERTY COMPILE_OPTIONS
                                                        "${_flag}")
    endif()
  endforeach()
endfunction()

# Appends flags to COMPILE_FLAGS of _target if supported by the C compiler. Note
# that it is simply not possible to pass different C and C++ flags, unless we
# drop support for the Visual Studio backend and use the COMPILE_LANGUAGE
...
option(DEVELOPER_MODE_FATAL_WARNINGS
       "Build with warnings as errors if DEVELOPER_MODE is also enabled" ON)
...
if(COMPILER_IS_GCC_OR_CLANG)
  ...
  # represented using cl.exe's options and should not be passed as flags, so we
  # do not add -fno-rtti or -fno-exceptions for clang-cl
  ...
  # Warnings to be disabled FIXME: We should probably not be disabling
  # -Wno-maybe-uninitialized?
  webkit_prepend_global_compiler_flags(
    -Qunused-arguments -Wno-maybe-uninitialized -Wno-parentheses-equality
    -Wno-misleading-indentation -Wno-psabi)

  # GCC < 12.0 gives false warnings for mismatched-new-delete
  ...
  # These GCC warnings produce too many false positives to be useful. We'll rely
  # on developers who build with Clang to notice these warnings.
  if(CMAKE_CXX_COMPILER_ID MATCHES "GNU")
    ...
    # This triggers warnings in wtf/Packed.h, a header that is included in many
    # places. It does not respect ignore warning pragmas and we cannot easily
    # suppress it for all affected files.
  ...
  # Makes builds faster. The GCC manual warns about the possibility that the
  # assembler being used may not support input from a pipe, but in practice the
  # toolchains we support all do.
  webkit_prepend_global_compiler_flags(-pipe)
endif()

if(COMPILER_IS_GCC_OR_CLANG AND NOT MSVC)
  # Don't give -Wall to clang-cl because clang-cl treats /Wall and -Wall as
  # -Weverything. -Wall and -Wextra should be specified before -Wno-* for Clang.
...
if("${CMAKE_CXX_COMPILER_ID}" STREQUAL "GNU"
   AND NOT "${LOWERCASE_CMAKE_HOST_SYSTEM_PROCESSOR}" MATCHES "x86_64")
  # To avoid out of memory when building with debug option in 32bit system. See
  # https://bugs.webkit.org/show_bug.cgi?id=77327
  set(CMAKE_SHARED_LINKER_FLAGS_DEBUG
      "-Wl,--no-keep-memory ${CMAKE_SHARED_LINKER_FLAGS_DEBUG}")
endif()
...
if(LTO_MODE AND COMPILER_IS_CLANG)
  ...
  set(CMAKE_SHARED_LINKER_FLAGS
      "-flto=${LTO_MODE} ${CMAKE_SHARED_LINKER_FLAGS}")
  set(CMAKE_MODULE_LINKER_FLAGS
      "-flto=${LTO_MODE} ${CMAKE_MODULE_LINKER_FLAGS}")
endif()
...
if(COMPILER_IS_GCC_OR_CLANG)
  ...
  if(ENABLE_SANITIZERS)
    if(MSVC AND WTF_CPU_X86_64)
      find_library(CLANG_ASAN_LIBRARY
                   clang_rt.asan_dynamic_runtime_thunk-x86_64 ${CLANG_LIB_PATH})
      find_library(CLANG_ASAN_RT_LIBRARY clang_rt.asan_dynamic-x86_64
                   PATHS ${CLANG_LIB_PATH})
      set(SANITIZER_LINK_FLAGS
          "\"${CLANG_ASAN_LIBRARY}\" \"${CLANG_ASAN_RT_LIBRARY}\"")
    else()
    ...
    set(CMAKE_EXE_LINKER_FLAGS
        "${CMAKE_EXE_LINKER_FLAGS} ${SANITIZER_LINK_FLAGS}")
    set(CMAKE_SHARED_LINKER_FLAGS
        "${CMAKE_SHARED_LINKER_FLAGS} ${SANITIZER_LINK_FLAGS}")
    set(CMAKE_MODULE_LINKER_FLAGS
        "${CMAKE_MODULE_LINKER_FLAGS} ${SANITIZER_LINK_FLAGS}")
  endif()
endif()
...
if(UNIX
   AND NOT APPLE
   AND NOT ENABLED_COMPILER_SANITIZERS)
  set(CMAKE_SHARED_LINKER_FLAGS
...
# Ensure that the default include system directories are added to the list of
# CMake implicit includes. This workarounds an issue that happens when using GCC
# 6 and using system includes (-isystem). For more details check:
# https://bugs.webkit.org/show_bug.cgi?id=161697
macro(DETERMINE_GCC_SYSTEM_INCLUDE_DIRS _lang _compiler _flags _result)
  file(WRITE "${CMAKE_BINARY_DIR}/CMakeFiles/dummy" "\n")
  separate_arguments(_buildFlags UNIX_COMMAND "${_flags}")
  execute_process(
    COMMAND ${CMAKE_COMMAND} -E env LANG=C ${_compiler} ${_buildFlags} -v -E -x
            ${_lang} -dD dummy
    WORKING_DIRECTORY ${CMAKE_BINARY_DIR}/CMakeFiles
    OUTPUT_QUIET
    ERROR_VARIABLE _gccOutput)
  file(REMOVE "${CMAKE_BINARY_DIR}/CMakeFiles/dummy")
  if("${_gccOutput}" MATCHES
     "> search starts here[^\n]+\n *(.+) *\n *End of (search) list")
...
if(COMPILER_IS_GCC_OR_CLANG)
  determine_gcc_system_include_dirs("c" "${CMAKE_C_COMPILER}"
                                    "${CMAKE_C_FLAGS}" SYSTEM_INCLUDE_DIRS)
  set(CMAKE_C_IMPLICIT_INCLUDE_DIRECTORIES
      ${CMAKE_C_IMPLICIT_INCLUDE_DIRECTORIES} ${SYSTEM_INCLUDE_DIRS})
  determine_gcc_system_include_dirs("c++" "${CMAKE_CXX_COMPILER}"
                                    "${CMAKE_CXX_FLAGS}" SYSTEM_INCLUDE_DIRS)
...
int main() {
  ...
  check_cxx_source_compiles("${FILESYSTEM_TEST_SOURCE}"
                            STD_FILESYSTEM_IS_AVAILABLE)
  if(NOT STD_FILESYSTEM_IS_AVAILABLE)
    ...
    check_cxx_source_compiles("${EXPERIMENTAL_FILESYSTEM_TEST_SOURCE}"
                              STD_EXPERIMENTAL_FILESYSTEM_IS_AVAILABLE)
    unset(CMAKE_REQUIRED_LIBRARIES)
  endif()
...
if(CMAKE_CXX_COMPILER_ID MATCHES "GNU" AND WTF_CPU_MIPS)
  # Work around https://gcc.gnu.org/bugzilla/show_bug.cgi?id=78176. This only
  # manifests when executing 32-bit code on a 64-bit processor. This is a
  # workaround and does not cover all cases (see comment #28 in the link above).
