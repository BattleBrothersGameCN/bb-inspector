...
macro(WEBKIT_COMPUTE_SOURCES _framework)
  ...
  foreach(_sourcesListFile IN LISTS ${_framework}_UNIFIED_SOURCE_LIST_FILES)
    configure_file("${CMAKE_CURRENT_SOURCE_DIR}/${_sourcesListFile}"
                   "${_derivedSourcesPath}/${_sourcesListFile}" COPYONLY)
    message(STATUS "Using source list file: ${_sourcesListFile}")

    list(APPEND _sourceListFileTruePaths
         "${CMAKE_CURRENT_SOURCE_DIR}/${_sourcesListFile}")
  endforeach()

  set(gusb_args --derived-sources-path ${_derivedSourcesPath}
                --source-tree-path ${CMAKE_CURRENT_SOURCE_DIR})
  # Windows needs a larger bundle size because that helps keep WebCore.lib's
  # size below the 4GB maximum in debug builds.
  if(MSVC
     AND ${_framework} STREQUAL "WebCore"
     AND ${_framework}_LIBRARY_TYPE STREQUAL "STATIC")
  ...
  if(ENABLE_UNIFIED_BUILDS)
    execute_process(
      COMMAND
        ${RUBY_EXECUTABLE} ${WTF_SCRIPTS_DIR}/generate-unified-source-bundles.rb
        ${gusb_args} "--print-bundled-sources" ${_sourceListFileTruePaths}
    ...
    foreach(_sourceFileTmp IN LISTS _outputTmp)
      set_source_files_properties(${_sourceFileTmp} PROPERTIES HEADER_FILE_ONLY
                                                               ON)
    ...
    execute_process(
      COMMAND
        ${RUBY_EXECUTABLE} ${WTF_SCRIPTS_DIR}/generate-unified-source-bundles.rb
        ${gusb_args} ${_sourceListFileTruePaths}
      RESULT_VARIABLE _resultTmp
      OUTPUT_VARIABLE _outputTmp)
    ...
    execute_process(
      COMMAND
        ${RUBY_EXECUTABLE} ${WTF_SCRIPTS_DIR}/generate-unified-source-bundles.rb
        ${gusb_args} "--print-all-sources" ${_sourceListFileTruePaths}
      RESULT_VARIABLE _resultTmp
      OUTPUT_VARIABLE _outputTmp)
...
macro(WEBKIT_FRAMEWORK_DECLARE _target)
  # add_library() without any source files triggers CMake warning Addition of
  # dummy "source" file does not result in any changes in generated build.ninja
  # file
...
macro(_WEBKIT_TARGET _target)
  target_sources(${_target} PRIVATE ${${_target}_HEADERS} ${${_target}_SOURCES})
  ...
  if(PLAYSTATION AND CMAKE_GENERATOR MATCHES "Visual Studio")
    ...
    set_source_files_properties(
      ${${_target}_SOURCES_C} PROPERTIES LANGUAGE C COMPILE_OPTIONS --std=gnu17)
  endif()

  target_include_directories(
    ${_target} PUBLIC "$<BUILD_INTERFACE:${${_target}_INCLUDE_DIRECTORIES}>")
  target_include_directories(
    ${_target} SYSTEM
    PRIVATE "$<BUILD_INTERFACE:${${_target}_SYSTEM_INCLUDE_DIRECTORIES}>")
  target_include_directories(
    ${_target}
    PRIVATE "$<BUILD_INTERFACE:${${_target}_PRIVATE_INCLUDE_DIRECTORIES}>")
  ...
  if(${_target}_PRIVATE_DEFINITIONS)
    target_compile_definitions(${_target}
                               PRIVATE ${${_target}_PRIVATE_DEFINITIONS})
  endif()
...
macro(_WEBKIT_TARGET_ANALYZE _target)
  if(ClangTidy_EXE)
    ...
    set_target_properties(
      ${_target} PROPERTIES C_CLANG_TIDY "${_clang_path_and_options}"
  ...
  if(IWYU_EXE)
    ...
    set_target_properties(${_target} PROPERTIES CXX_INCLUDE_WHAT_YOU_USE
                                                "${_iwyu_path_and_options}")
  endif()
endmacro()

function(_WEBKIT_TARGET_LINK_FRAMEWORK_INTO target_name framework
         _public_frameworks_var _private_frameworks_var)
  set_property(GLOBAL PROPERTY ${framework}_LINKED_INTO ${target_name})

  get_property(_framework_public_frameworks GLOBAL
               PROPERTY ${framework}_FRAMEWORKS)
  foreach(dependency IN LISTS ${_framework_public_frameworks})
    set(${_public_frameworks_var}
        "${${_public_frameworks_var}};${dependency}"
        PARENT_SCOPE)
  endforeach()

  get_property(_framework_private_frameworks GLOBAL
               PROPERTY ${framework}_PRIVATE_FRAMEWORKS)
  foreach(dependency IN LISTS _framework_private_frameworks)
    set(${_private_frameworks_var}
        "${${_private_frameworks_var}};${dependency}"
        PARENT_SCOPE)
    _webkit_target_link_framework_into(
      ${target_name} ${dependency} ${_public_frameworks_var}
      ${_private_frameworks_var})
  endforeach()
endfunction()
...
macro(_WEBKIT_FRAMEWORK_LINK_FRAMEWORK _target_name)
  ...
  set_property(GLOBAL PROPERTY ${_target_name}_PUBLIC_LIBRARIES
  ...
  if(${_target_name}_LIBRARY_TYPE STREQUAL "SHARED")
    ...
    foreach(framework IN LISTS _private_frameworks)
      _webkit_target_link_framework_into(${_target_name} ${framework}
                                         _public_frameworks _private_frameworks)
    endforeach()
  endif()
  ...
  foreach(framework IN LISTS _private_frameworks)
    if(${_target_name}_LIBRARY_TYPE STREQUAL "SHARED")
      get_property(_linked_libraries GLOBAL
                   PROPERTY ${framework}_PUBLIC_LIBRARIES)
      list(APPEND ${_target_name}_INTERFACE_LIBRARIES ${_linked_libraries})
      ...
      if(${framework}_LIBRARY_TYPE STREQUAL "OBJECT")
        list(APPEND ${_target_name}_PRIVATE_LIBRARIES
  ...
  set_property(GLOBAL PROPERTY ${_target_name}_PRIVATE_FRAMEWORKS
                               ${_private_frameworks})
endmacro()
...
macro(_WEBKIT_TARGET_LINK_FRAMEWORK _target)
  foreach(framework IN LISTS ${_target}_FRAMEWORKS)
    ...
    # See if the target is linking a framework that the specified framework is
    # already linked into
    if((NOT _linked_into)
       OR (${framework} STREQUAL ${_linked_into})
       OR (NOT ${_linked_into} IN_LIST ${_target}_FRAMEWORKS))
...
macro(_WEBKIT_LIBRARY_LINK_FRAMEWORK _target)
  # See if the library is SHARED and if so just link frameworks the same as
  # executables
...
macro(_WEBKIT_TARGET_INTERFACE _target)
  ...
  target_link_libraries(${_target}_PostBuild
                        INTERFACE ${${_target}_INTERFACE_LIBRARIES})
  target_include_directories(
    ${_target}_PostBuild INTERFACE ${${_target}_INTERFACE_INCLUDE_DIRECTORIES})
  ...
  if(NOT ${_target}_LIBRARY_TYPE STREQUAL "SHARED")
    target_compile_definitions(${_target}_PostBuild
                               INTERFACE "STATICALLY_LINKED_WITH_${_target}")
  endif()
...
macro(WEBKIT_FRAMEWORK _target)
  ...
  if(${_target}_OUTPUT_NAME)
    set_target_properties(${_target} PROPERTIES OUTPUT_NAME
                                                ${${_target}_OUTPUT_NAME})
  endif()

  if(${_target}_PRE_BUILD_COMMAND)
    add_custom_target(
      _${_target}_PreBuild
      COMMAND ${${_target}_PRE_BUILD_COMMAND}
      VERBATIM)
  ...
  if(${_target}_POST_BUILD_COMMAND)
    add_custom_command(
      TARGET ${_target}
      POST_BUILD
      COMMAND ${${_target}_POST_BUILD_COMMAND}
      VERBATIM)
  endif()

  if(APPLE
     AND NOT PORT STREQUAL "GTK"
     AND NOT ${${_target}_LIBRARY_TYPE} MATCHES STATIC)
...
macro(WEBKIT_EXECUTABLE _target)
  ...
  if(WIN32)
    ...
    target_link_options(
      ${_target}
      PRIVATE
      "/manifestdependency:type='win32' name='Microsoft.Windows.Common-Controls' version='6.0.0.0' processorArchitecture='${_processor_architecture}' publicKeyToken='6595b64144ccf1df' language='*'"
    )
  endif()
endmacro()
...
# Append the given flag to the target property. Builds on top of
# get_target_property() and set_target_properties()
...
macro(WEBKIT_CREATE_SYMLINK target src dest)
  add_custom_command(
    TARGET ${target}
    POST_BUILD
    COMMAND ln -sf ${src} ${dest} DEPENDS ${dest}
